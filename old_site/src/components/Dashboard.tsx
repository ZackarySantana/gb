...
export default function Dashboard() {
    ...
    const initialCommit =
        (Array.isArray(params.commit) ? params.commit[0] : params.commit) ||
        (Array.isArray(searchParams.commit)
            ? searchParams.commit[0]
            : searchParams.commit) ||
        "";
    const initialBenchmark =
        (Array.isArray(params.benchmark)
            ? params.benchmark[0]
            : params.benchmark) ||
        (Array.isArray(searchParams.benchmark)
            ? searchParams.benchmark[0]
            : searchParams.benchmark) ||
        "";
    const initialMetric =
        (Array.isArray(params.metric) ? params.metric[0] : params.metric) ||
        (Array.isArray(searchParams.metric)
            ? searchParams.metric[0]
            : searchParams.metric) ||
        "";
    ...
    createEffect(() => {
        ...
        if (benchName() && benchName() !== "")
            urlParams.benchmark = benchName();
    ...
    const [run] = createResource<RunFile | undefined, string | undefined>(
        selectedRunPath,
        async (p) => {
        ...
        for (let i = 0; i < last.length; i++) {
            ...
            try {
                ...
                if (!isNaN(parsedTimestamp)) {
                    ...
                    timestamp =
                        new Date(runFile.date + "T12:00:00Z").getTime() / 1000;
                }
            } catch (error) {
                // Fallback to old RunFile format
                    ...
                    <div class="section-filters">
                        <div class="section-filter">
                            ...
                            <select
                                ...
                                value={benchName() || ""}
                                onChange={(e) =>
                                    setBenchName(e.currentTarget.value)
                                }
                                class="filter-select"
                            >
                                <option value="">Select benchmark...</option>
                                <Show
                                    when={benchOptions().length > 0}
                                    fallback={<option>Loading...</option>}
                                >
